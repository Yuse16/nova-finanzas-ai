...
import { useUI } from '@/lib/ui-context' // Importar useUI

const actions: { label: string; sub: string; icon: LucideIcon; kind: string | null; initialData?: any; color: string }[] =
  [
    { label: 'Registrar', sub: 'por voz', icon: Mic, kind: null, color: 'oklch(0.7 0.18 295)' }, // kind null porque se usa onVoice
    { label: 'Registrar', sub: 'por texto', icon: PenLine, kind: 'transaction', color: 'oklch(0.74 0.15 235)' },
    { label: 'Escanear', sub: 'ticket', icon: ScanLine, kind: 'transaction', color: 'oklch(0.74 0.15 175)' }, // Asumo que abrirá el modal de transacción
    ...
    <div className="-mx-5 flex gap-3 overflow-x-auto px-5 pb-1 no-scrollbar">
      {actions.map((a, i) => (
        <GlassCard
          ...
          whileTap={{ scale: 0.94 }}
          onClick={
            a.kind === null // Si kind es null, es por voz
              ? onVoice
              : () => open(a.initialData ? { kind: a.kind, initialData: a.initialData } : { kind: a.kind })
          }
